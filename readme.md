...
- [eunomia-bpf](https://github.com/eunomia-bpf/eunomia-bpf) - A compile SDK and runtime library for build, distribute, dynamically load and run CO-RE eBPF app in multi languages and Webassembly. It supports writing eBPF kernel code only to build simple CO-RE libbpf eBPF applications, writing kernel part in both BCC and libbpf style, writing userspace in multiple languages in WASM module and distribute with WASM OCI images. The runtime is based on libbpf only, provided CO-RE to BCC style eBPF program without llvm dependence.
